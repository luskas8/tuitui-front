...
export function Button ({ title, icon, type = 'button', disabled, onClick }: ButtonProps) {
  return (
        <button
            ...
            disabled={disabled}
            type={type}
            className='w-full flex justify-center items-center gap-[10px] px-4 py-3 bg-dark-blue text-white rounded hover:bg-purple active:bg-dark-black active:outline-none focus:outline focus:outline-4 focus:outline-purple disabled:bg-slate-400'
        >
            {icon && icon}
            {title}
        </button>
